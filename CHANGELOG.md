...
- **Bestillings-webdelen (Bestillingsportalen/ProjectProvision) er flyttet ut av `pp-portfolio-web-parts`** og vedlikeholdes nå i [Bestillingsportalen-repoet](https://github.com/Puzzlepart/bestillingsportalen) (fra Bestillingsportalen, pakken `bp-provision-web-parts`, samme komponent-id). Tenanter som bruker bestillingsflaten **må distribuere Bestillingsportalen ≥ 2.1.0 rett etter denne oppgraderingen** — `Bestillingsportalen.aspx`-siden viser «finner ikke komponenten» i mellomtiden. Rekkefølgen er obligatorisk (PP365 først, deretter Bestillingsportalen — appkatalogen avviser to pakker med samme komponent-id), se Bestillingsportalens `Upgrade.md`. Idémodulens innebygde bestillingsknapp og webdelegenskapen `provisionUrl` er fjernet. PnP-malen provisjonerer ikke lenger `Bestillingsportalen.aspx` eller «Bestillingsportalen»-sikkerhetsgruppen på nye porteføljer; eksisterende sider og grupper røres ikke (merk at gruppen heter `Provision portal` på engelskspråklige installasjoner fra før). Skal webdelens tilgangsstyring brukes på en ny portefølje, opprettes gruppen manuelt på området der webdelen ligger — se Bestillingsportalens `Configuration-guide.md`. Forbedringene og feilrettingene for bestillingsskjemaet som opprinnelig var planlagt i denne versjonen - standard metadata fra prosjekttypen, ekskludering av områdetyper per instans, tydeligere `DefaultHub`-håndtering og `MinimumOwners`-validering - inngår nå i Bestillingsportalen 2.1.0 og er dokumentert der.
