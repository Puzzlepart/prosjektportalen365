...
- **Modernisert byggekjede og testregime (ingen funksjonell endring tilsiktet).** SPFx er oppgradert fra 1.17.4 til 1.23.2 (Heft i stedet for gulp, Node 22, TypeScript 5.8, ESLint 9) og PnPjs fra 3.17 til 4.21 i alle løsningene; taksonomimodulen PnPjs 4 fjernet er portet inn i det delte biblioteket, `sp-entityportal-service` er tatt inn i det delte biblioteket og `sp-js-provisioning` er oppgradert til 1.4.0. Termetiketter følger nå én regel overalt: områdets språk, deretter `nb-NO`, deretter `en-US`. Bundlene inneholder én PnPjs-versjon i stedet for tre. Enhets-, komponent- og kjøretidstester kjører i hvert bygg, Playwright-røyktester kjører mot testkanalen etter hver utrulling, og utgivelsesbygget verifiserer at hver pakke faktisk produseres. Bygging fra kildekode krever Node 22
...
- **Prosjektveiviseren versjon 6.** Hele malverket er oppdatert til Prosjektveiviseren v6 (Digdir, juni 2026): nye v6-fasesjekkpunkter og -prosjektoppgaver i egne hub-lister med valgbart innholdssett per prosjektmal, terminologiskifte gevinst → nytte i alle brukervendte tekster, nye statusseksjoner for omfang og bærekraft, v6-dokumentmaler og v6-fasetekster, samt egen oppgraderingssti med opt-in-bryter (`-GevinstTilNytte`) for nytteterminologi på eksisterende prosjektområder. Se releasenotes for 1.14.0 for detaljer. [#1044](https://github.com/Puzzlepart/prosjektportalen365/issues/1044)
