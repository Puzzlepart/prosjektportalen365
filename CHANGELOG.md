...
- **Prosjektveiviseren versjon 6.** Hele malverket er oppdatert til Prosjektveiviseren v6 (Digdir, juni 2026). [#1044](https://github.com/Puzzlepart/prosjektportalen365/issues/1044)
  - **Nytt innhold side om side:** v6-fasesjekkpunkter (63) og v6-prosjektoppgaver (26, med beskrivelse og Planner-sjekklister) provisjoneres i nye hub-lister (`Lists/Fasesjekklistev6`, `Lists/Planneroppgaverv6`) med de opprinnelige visningsnavnene, mens eksisterende lister beholdes urørt som «… (tidligere)». Virksomheten velger innholdssett per prosjektmal via `Listeinnhold`; ren installasjon får v6 som standard, oppgradering beholder virksomhetens eksisterende valg.
  - **Terminologi gevinst → nytte:** alle brukervendte tekster følger v6-/DFØ-terminologien (nyttevirkninger, nyttestyring, Nytteoversikt, Nytteanalyse og plan for nyttestyring, Nytteoppnåelse). Rollefeltene heter nå «Nytteeier» (tidligere Gevinstansvarlig) og «Nytteansvarlig» (tidligere Gevinsteier). Interne feltnavn, liste-URL-er, sidefilnavn og lagrede data er uendret.
  - **Bærekraft og omfang som styringsparametere:** to nye statusseksjoner i prosjektstatus (`GtStatusScope`, `GtStatusSustainability`) med kommentarfelt, valgverdier og fargekonfigurasjon — i tråd med v6s sju styringsparametere.
  - **v6-dokumentmaler:** `Malbibliotek/Fra Prosjektveiviseren` inneholder nå de 11 v6-malene fra Digdir, inkludert «Plan for nyttestyring» og den nye malpakken for små prosjekter (forenklet styringsdokument/prosjektkanvas). Det gamle malsettet (v2.2–v5.0) provisjoneres ikke lenger; eksisterende filer hos oppgraderte virksomheter beholdes og kan slettes manuelt.
  - **v6-fasetekster** (undertekster og fasebeskrivelser) i termlageret på begge språk. Gjelder nye installasjoner — fasetermene oppdateres ikke ved oppgradering; virksomheter som ønsker de nye tekstene kan oppdatere `PhaseSubText`/`PhaseDescription` manuelt i termlageret.
  - **Oppgraderingssti:** `-Upgrade` omdøper hub-listene og berørte konfigurasjonsrader på plass (ingen duplikater, ingen re-kobling av maloppsett), og `UpgradeAllSitesToLatest.ps1` har fått ny valgfri bryter `-GevinstTilNytte` som oppdaterer visningsnavn fra gevinst- til nytteterminologi på eksisterende prosjektområder (opt-in; uten bryteren beholder prosjektområdene dagens terminologi).
  - Arbeidsdokumentasjon og revisjonsspor: `docs/prosjektveiviseren-v6/`.
...
- `Prosjektutlisting` viser nå innlastnings-idikator mens data lastes inn i stedet for en blank side. Gjelder både kort- og listevisning
