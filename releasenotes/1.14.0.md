...
- **[Forbedringer i Bestillingsportalen](#forbedringer-i-bestillingsportalen)** - Standard metadata fra prosjekttypen, bedre hubhåndtering og mer.

## Sentral malpakkekatalog

Den store nyheten i denne versjonen er den sentrale malpakkekatalogen. Administratorer kan nå bla i og installere malpakker - maler, prosjekttillegg og innholdspakker - direkte fra hubområdet via den nye `Malpakkekatalog`-kommandoen.
...
## Forbedringer i Bestillingsportalen

Bestillingsportalen har fått flere forbedringer i denne versjonen:

- **Standard metadata fra prosjekttypen**: Bestillingsskjemaet bruker nå `DefaultMetadata` fra prosjekttypen til å sette prosjektegenskaper og valgt innholdstype på `Prosjektdata`-elementet som opprettes ved bestilling. En forhåndsvisning av verdiene vises i siste steg av bestillingsprosessen
- **Ekskluder områdetyper per instans**: Administrator kan nå ytterligere ekskludere områdetyper i egenskapspanelet for bestillingsskjemaet, slik at enkelte områdetyper kan skjules fra brukerne per instans av skjemaet
- **Tydeligere hubhåndtering**: Appen viser nå en advarsel på hubfeltet dersom standardhuben (`DefaultHub`) som er angitt på prosjekttypen ikke kan slås opp, i stedet for å falle tilbake til gjeldende hub uten tilbakemelding. I tillegg er en rekke feil rundt `DefaultHub`-håndtering rettet - blant annet ved lagring uten hubtilknytning, ved bytte av områdetype, og ved bestilling av underområder mot en annen hub

<!-- ![image](./assets/1.14/bestillingsportalen-forhandsvisning.png) -->
