# Prosjektportalen 365 - 1.14.0 (September 2026)
...
Fasesjekkpunktene er oppdatert med de nye og endrede fasesjekkpunktene fra Digdir. Planneroppgavene har fått en større oppgradering. Isteden for at hver oppgave matcher et sjekkpunkt - som førte til svært mange Planner-oppgaver per prosjekt - har vi forsøkt å redusere antall oppgaver, og heller bruke sjekklister i oppgavene. Nå beskriver hver oppgave i større grad de ulike temaene i hver fase. v6-fasesjekkpunktene (63) og v6-prosjektoppgavene (26, med beskrivelser og Planner-sjekklister) provisjoneres i nye hub-lister (`Lists/FasesjekklisteV6`, `Lists/PlanneroppgaverV6`) med de opprinnelige visningsnavnene, mens de eksisterende listene beholdes urørt som «… (tidligere)». Virksomheten velger selv innholdssett per prosjektmal via `Listeinnhold` - ren installasjon får v6 som standard, mens oppgradering beholder virksomhetens eksisterende valg.
...
Prosjektstatus har fått to nye statusseksjoner - omfang (`GtStatusScope`) og bærekraft (`GtStatusSustainability`) - med kommentarfelt, valgverdier og fargekonfigurasjon, i tråd med v6s sju styringsparametere. Bærekraft har i tillegg valget «Ikke tatt stilling til» for prosjekter som ennå ikke har vurdert bærekraftskravene.

<!-- TODO skjermbilde: En publisert statusrapport der seksjonene Omfang og Bærekraft vises sammen med de øvrige styringsparameterne -->
![image](./assets/1.14/prosjektstatus-omfang-baerekraft.png)

**Dokumentmaler:**

`Malbibliotek/Fra Prosjektveiviseren` inneholder nå de 11 v6-malene fra Digdir, inkludert «Plan for nyttestyring», «Nytteberegninger» og den nye malpakken for små prosjekter (forenklet styringsdokument/prosjektkanvas og forenklet plan for nyttestyring). Det gamle bokmålsmalsettet (v2.2–v5.0) provisjoneres ikke lenger - eksisterende filer hos oppgraderte virksomheter beholdes og kan slettes manuelt. Undermappen `Nynorsk` beholder inntil videre de nynorske malene fra forrige versjon av Prosjektveiviseren, siden Digdir foreløpig ikke tilbyr nynorske v6-maler.

**Fasetekster i termlageret:**
...
En annen stor nyhet i denne versjonen er den sentrale malpakkekatalogen. Administratorer kan nå bla i og installere malpakker - maler, prosjekttillegg og innholdspakker - direkte fra hubområdet via den nye `Malpakkekatalog`-kommandoen i kommandolinjen på `Maloppsett`-listen.
...
- **Styring fra pakken**: En malpakke kan låse prosjekttillegg til malen sin, slik at de alltid legges på og ikke kan velges bort i oppsettveiviseren, og kan angi egen innholdstype for prosjektstatus (`GtProjectStatusContentType`) for prosjekter som opprettes fra malen

Importerte pakkers oppsettfiler lagres i den reserverte mappen `pp-packages` i `Malbibliotek`. Mappen og innholdet i den skjules for `Hent dokumentmal`, siden filene er input til oppsettveiviseren og ikke dokumentmaler.

<!-- TODO skjermbilde: Malpakkekatalog-skuffen åpnet fra Maloppsett-listen, med pakkeliste, søk og filtre -->
![image](./assets/1.14/malpakkekatalog.png)

<!-- TODO skjermbilde: Detaljvisning for en pakke med skjermbilder, krav/avhengigheter, innholdsoversikt og versjonshistorikk -->
![image](./assets/1.14/malpakkekatalog-detaljer.png)

<!-- TODO skjermbilde: Stegvis fremdriftsvisning under import av en pakke -->
![image](./assets/1.14/malpakkekatalog-import.png)
...
| Før                                           | Etter                                             |
| --------------------------------------------- | ------------------------------------------------- |
| ![image før](./assets/1.14/omrademal-for.png) | ![image etter](./assets/1.14/omrademal-etter.png) |

**Innholdstyper fra malene:**

Innholdstypene opprettes nå direkte fra JSON-malene med faste innholdstype-ID-er, via `sp-js-provisioning` `1.3.15`. De 15 site scriptene som tidligere kun opprettet tomme innholdstyper er fjernet, slik at innholdstypene forvaltes ett sted - i malene - med fullstendige metadata og feltreferanser.
...
- **Vis avsluttede prosjekter i Prosjektutlisting**: Ny innstilling som lar vertikalene vise prosjekter med prosjektstatus `Avsluttet`. Tidligere ble disse fjernet før vertikalene ble filtrert. Innstillingen er av som standard
- **Raskere bunntekst**: `Footer` på prosjekt- og porteføljeområder henter nå GitHub-releaser, hjelpeinnhold, lenker og favorittprosjekter først når den aktuelle knappen åpnes, i stedet for ved hver sideinnlasting
