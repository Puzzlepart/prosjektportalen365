...
- **[Modernisert teknisk grunnmur](#modernisert-teknisk-grunnmur)** - Ny byggekjede (SPFx 1.23, Node 22), PnPjs 4 og automatiske tester i hvert bygg og mot testmiljøet. Ingen funksjonell endring, men et solid fundament for videre utvikling.
...
- **Ny byggekjede.** Alle løsningene bygges nå med SharePoint Framework 1.23 på Microsofts nye Heft-verktøykjede, Node 22 og TypeScript 5.8. Den gamle gulp-baserte kjeden og Node 16 er faset ut. Dette holder Prosjektportalen på en støttet plattform og er en forutsetning for de neste oppgraderingene (Fluent UI v9, React 18).
- **PnPjs 4.** Databiblioteket som alle webdelene bruker mot SharePoint er løftet fra versjon 3 til 4. Underveis er to eksterne pakker som ikke lenger vedlikeholdes tatt inn i vår egen kode, og provisjoneringsmotoren `sp-js-provisioning` er oppgradert til 1.4.0. Hver webdel-pakke inneholder nå én PnPjs-versjon i stedet for tre, som gir mindre pakker.
...
- **Automatiske tester.** Enhets- og komponenttester kjører i hvert bygg, og et sett Playwright-tester åpner huben og et prosjektområde i testmiljøet etter hver utrulling og sjekker at webdelene faktisk laster. Den første kjøringen fant og stoppet en feil før den nådde noen kunde. Den manuelle smoke-testen beholdes for flytene som skriver til SharePoint, til de også er automatisert.

**Hva betyr det for deg?** Ingenting ved installasjon eller oppgradering med utgivelsespakken endres; krav til PnP.PowerShell er som før. Bygger du Prosjektportalen selv fra kildekoden, trenger du Node 22 (`.nvmrc` i repoet angir versjonen). Meld gjerne fra om noe ser eller oppfører seg annerledes enn i 1.14 — det er per definisjon en feil i denne versjonen.
