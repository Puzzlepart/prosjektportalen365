...
2. **Portfolio flag filter** — `filterByShowInPortfolio` toggle (default **on**): only items whose `GtShowInPortfolioOWSBOOL` managed property is truthy (`'1'`/`'true'`) are shown in data source mode, so child projects control what bubbles up via the existing "Vis i porteføljen" field. Toggle off for parity with the aggregation list page.
...
5. **Apps-only deployment** — no Templates changes at all in this repository. A **Prosjekttillegg** provisioning a `Usikkerhetsmatrise.aspx` page with both matrices in forced `dataSource` mode is intentionally *not* part of this change — it can be distributed separately (e.g. per-tenant, or via the malpakkekatalog) as a JSON add-on uploaded to a hub's Prosjekttillegg library. Until then the page is created manually by adding the two web parts.
...
6. Manually created page with both matrices in forced `dataSource` mode and `showTitle` enabled → titles distinguish the stacked matrices. (A separately distributed Prosjekttillegg can automate this page — not part of this change.)
