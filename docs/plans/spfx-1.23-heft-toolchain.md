...
- **Third-party CSS hashed as CSS modules.** The rig's webpack config treats every `.css` not named `*.global.css` as a CSS module, so `react-calendar-timeline/lib/Timeline.css` and `fabric.min.css` got hashed class names and the project timeline rendered as an unclickable overlay (its DOM uses the plain names). Fixed in `config/spfx-customize-webpack.js` (`treatNodeModulesCssAsGlobal`): node_modules stylesheets bypass the module rule and use the rig's global-CSS loaders. Found by manual smoke testing; the Playwright suite only checked that the timeline web part mounted. The first version of the fix over-matched: consumers bundle `pp365-shared-library` from `node_modules`, so its `*.module.scss.css` files were compiled as global stylesheets too and `styles` became undefined in every shared component (`reading 'accordionChevron'`). The CI e2e run failed on that build, but only after it had been deployed to the test tenant. The rule now applies to third-party stylesheets only. Follow-ups added: trial-click and panel-open assertions in the smoke tests, an error-boundary check on every smoke page, SPFx's "Could not load … in require" treated as fatal, and a release-build guard against hashed third-party class names.

- **`Buffer is not defined` in the template package catalog.** webpack 5 no longer polyfills Node globals; `xml-js` (sp-js-provisioning's XML serializer) tests `json instanceof Buffer`, so provisioning the first list from a catalog package threw. Fixed with the `buffer` package as a devDependency of PortfolioExtensions and ProjectExtensions and `webpack.ProvidePlugin` in `spfx-customize-webpack.js` (`provideNodeGlobals`). Found by manual test of the catalog install (a write flow outside the browser suite).
