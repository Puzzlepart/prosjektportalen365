...
- **Theme/UI:** `import { customLightTheme, getFluentIcon, UserMessage } from 'pp365-shared-library'`; wrap tree in `<IdPrefixProvider value={useId(...)}><FluentProvider theme={customLightTheme}>` — as implemented in [TemplatePackageCatalog.tsx](SharePointFramework/PortfolioExtensions/src/components/TemplatePackageCatalog/TemplatePackageCatalog.tsx).
- **State:** Context + `useState` shallow-merge hook (see [useTemplatePackageCatalogState.ts](SharePointFramework/PortfolioExtensions/src/components/TemplatePackageCatalog/useTemplatePackageCatalogState.ts) + [context.ts](SharePointFramework/PortfolioExtensions/src/components/TemplatePackageCatalog/context.ts)). **No RTK.**
- **Styling:** `*.module.scss` using CSS-var tokens (`var(--colorNeutral*)`), responsive `@media` — see [TemplatePackageCatalog.module.scss](SharePointFramework/PortfolioExtensions/src/components/TemplatePackageCatalog/TemplatePackageCatalog.module.scss). `.scss.ts` typings are build-generated — do not hand-author.
...
- **`PackageCard/`** — selectable Fluent `Card` (see [PackageCard.tsx](SharePointFramework/PortfolioExtensions/src/components/TemplatePackageCatalog/PackageCard/PackageCard.tsx)): `CardPreview` thumbnail (fallback on error), `name`, status `Badge` (Installert/Oppdatering/Sentral), `TagGroup`/`Tag` from `tags`, footer meta (`v{version}` · `publishedDate` · `author`), `aria-selected`. No downloads/"Anbefalt" badge (not in catalog).
