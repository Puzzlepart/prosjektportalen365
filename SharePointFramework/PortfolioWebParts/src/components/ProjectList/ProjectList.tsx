...
import { SiteContext, Toolbar, UserMessage, customLightTheme } from 'pp365-shared-library'
...
export const ProjectList: FC<IProjectListProps> = (props) => {
    ...
    if (context.state.error) {
        return (
            <FluentProvider theme={customLightTheme}>
                <section className={styles.projectList}>
                    <UserMessage title={strings.ErrorFetchingProjectsTitle} text={context.state.error} intent='error' />
                </section>
            </FluentProvider>
        )
    }
            ...
            <IdPrefixProvider value={context.fluentProviderId}>
                <FluentProvider className={styles.projectList} theme={customLightTheme}>
                    <div className={styles.tabs}>
                        <TabList
                            onTabSelect={(_, data: SelectTabData) => context.setState({ selectedVertical: find(context.verticals, (v) => v.key === data.value) })}
