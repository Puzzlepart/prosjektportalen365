...
import { AppsListFilled, AppsListRegular, bundleIcon, GridFilled, GridRegular, TextBulletListLtrFilled, TextBulletListLtrRegular, TextSortAscendingFilled, TextSortAscendingRegular, TextSortDescendingFilled, TextSortDescendingRegular } from '@fluentui/react-icons'
...
export function useToolbarItems(
  ...
  useEffect(
    () => setState({ selectedRenderMode: find(renderModes, (mode) => mode.value === state.renderMode) }),
    [state.renderMode]
  )
  ...
  const menuItems = useMemo<ListMenuItem[]>(
    () =>
      [
        ...
        (props.showSortBy && state.renderMode === 'tiles') &&
