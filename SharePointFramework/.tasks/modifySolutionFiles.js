 ...
 * 
 * The script is run as a pre-build script when the serve channel is not main. This is to avoid
 * generating temporary files when the main channel is being served. The script is also run as a
 * post-watch script to revert the package solution file and component manifest files to the
 * backup files.
 * 
 * The script can be run with the following parameters:
 * `--revert` - Reverts the package solution file and component manifest files to the backup files
 * `--force` - Forces the script to run even if no serve channel is specified
 */
const argv = require('yargs').argv
const fs = require('fs')
const glob = require('glob').sync
const isRevert = argv.revert
 ...
 */
(async () => {
    if (process.env.SERVE_CHANNEL === 'main' || (!process.env.SERVE_CHANNEL && !isForce)) {
        return
    }
    const componentManifestFiles = glob(joinPath(process.cwd(), `src/**/manifest.json`))
    if (isRevert || isPostWatch) {
