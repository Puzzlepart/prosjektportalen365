...
export default class ProjectSetup extends BaseApplicationCustomizer<IProjectSetupProperties> {
  ...
  private async _fetchData(): Promise<IProjectSetupData> {
    try {
      ...
      const templateFileName = (await sp.web.select("Title", "AllProperties").expand("AllProperties").get())['AllProperties']['pptemplate'];
      let templateViewXml = '<View Scope="RecursiveAll"><Query><Where><Eq><FieldRef Name="FSObjType" /><Value Type="Integer">0</Value></Eq></Where></Query></View>';
      if (templateFileName) {
