define([], function () {
  return {
    EditTimelineContentText: "Edit item",
    NewTimelineContentText: "New item",
    TimelineEndDateMilestoneDescription: "End date is required for element types displayed as a point on the timeline ({0})",
    NewStatusPanelTitle: "New status report",
    EditStatusPanelText: "Edit status report",
    ErrorBoundaryFallbackDescription: "Error details:",
    ErrorBoundaryFallbackText: "An error occurred while loading {0}.",
    ArchiveLogStatusSuccess: 'Archived',
    ArchiveLogStatusError: 'Error',
    ArchiveLogStatusWarning: 'Warning',
    ArchiveLogStatusInProgress: 'To archive',
    ArchiveLogStatusSuccessTooltip: 'The item was archived successfully in the last archive run.',
    ArchiveLogStatusErrorTooltip: 'The item failed during the last archive run and may need to be retried.',
    ArchiveLogStatusWarningTooltip: 'The item was processed during the last archive run, but with warnings.',
    ArchiveLogStatusInProgressTooltip: 'The item is queued for archiving and will be picked up by the archiving flow.',
    ArchiveLogScopeDocument: 'Document',
    ArchiveLogScopeList: 'List',
    ArchiveLogOperationArchive: 'Archive',
    ProjectPropertiesTitleLabel: 'Title',
    ArchiveLogOperationPhaseTransition: 'Phase Transition',
    ArchiveLogOperationManual: 'Manual archive',
    ArchiveLogOperationEndClosed: 'End/Closed/Locked',
    UpdatingProjectPropertiesErrorText: "An error occurred while saving project properties. Please try again later.",
    ChangePhaseDialogTitle: "Change to {0} phase",
    ChangePhaseDialogSubtitle: "This will change the phase from {0} to {1}",
    ChangeFirstPhaseDialogSubtitle: "This will set the project to the {0} phase",
    ChecklistMandatory: "Checklist items for changing to this phase are mandatory and cannot be skipped.",
    ProjectTimeLapseLabel: "Center timeline on project duration",
    DeleteReportButtonLabel: "Delete",
    DeleteReportButtonDescription: "Delete the selected status report.",
    DeleteReportButtonDescriptionNoPermission: "You do not have permission to delete this status report.",
    EditReportButtonDescription: "Edit the selected status report.",
    EditReportButtonDescriptionNoPermission: "You do not have permission to edit this status report.",
    ErrorTitle: "An error has occurred",
    ExternalUsersConfigInfoTitle: "External users/guests",
    GetSnapshotButtonDescription: "Opens the selected status report as a snapshot.",
    NewStatusReportDescription: "Create a new status report for the project.",
    NewStatusReportDescriptionNoPermission: "You do not have permission to create a new status report.",
    NoPropertiesTitle: "Project information is missing.",
    NoReportsFoundTitle: "No reports found",
    PhaseSitePageTitle: "Dynamic homepage is enabled",
    PhaseUseStartArrowLabel: "Use start arrow",
    PhaseUseEndArrowLabel: "Use end arrow",
    ProjectInformationStatusReportHeaderDescription: "Published ({0})",
    ProjectPropertiesHeader: "Project information",
    PublishReportButtonDescription: "Publishes the selected status report.",
    PublishReportButtonDescriptionNoPermission: "You do not have permission to publish this status report.",
    ShowFieldExternalUsersOffText: "Not visible to external users",
    ShowFieldExternalUsersOnText: "Visible to external users",
    ShowFilterButtonLabel: "Show filter button",
    ShowInfoMessageLabel: "Show info message",
    ShowPhaseSitePageMessageLabel: "Show info message (dynamic homepage)",
    PublishedStatusReportInfo: "The status report is published and cannot be edited/deleted.",
    AlreadyPublishedReportInfo: "The status report is already published.",
    PhaseCheckListItem: "Checklist item {0}",
    Aria: {
      CurrentPhaseText: 'This is the current phase. It is therefore not possible to change to this phase.',
    },
    SaveText: 'Save',
    UpdatingProjectPropertiesStatusText: 'Updating project properties ...',
    SynchronizingProjectPropertiesToPortfolioSiteStatusText:
      'Synchronizing project properties to portfolio site ...',
    AdminPageLinkLabel: 'Path to Admin page',
    AdvancedGroupName: 'Advanced',
    CalloutTemplateFieldLabel: 'Callout template',
    CancelText: 'Cancel',
    ConfirmText: 'Confirm',
    CategoryFieldLabel: 'Category',
    ChangePhaseText: 'Change to this phase',
    ChangingPhaseDescription: 'Changing phase to {0}',
    ChecklistCommentDescription:
      'Comment is mandatory if the checkpoint is to be marked as "Not relevant" or "Still open".',
    ChecklistCommentPlaceholder: 'Enter a comment...',
    CheckpointDoneTooltip: 'Checkpoint completed',
    CheckpointNotRelevantTooltip: 'Checkpoint is not relevant',
    CheckpointNotRelevantTooltipCommentEmpty:
      "You cannot set the checkpoint as 'Not Relevant' without commenting.",
    CheckPointsStatus: '{0} checkpoints: **{1}**',
    CheckpointStillOpenTooltip: 'Checkpoint is still open',
    CheckpointStillOpenTooltipCommentEmpty:
      "You cannot set the checkpoint as 'Still open' without posting a comment.",
    ChildProjectAdminLabel: 'Manage childprojects',
    CloseText: 'Close',
    ColorPickerStrings: {
      blue: 'Blue',
      green: 'Green',
      hex: '',
      red: 'Red'
    },
    CommentLabel: 'Comment',
    ConfigItemTitleFieldLabel: 'Element title (configuration)',
    ConfirmChangePhase: 'Do you want to change phase to {0}?',
    ConsequenceFieldNameFieldLabel: 'Field name - Consequence',
    ConsequenceHeaderFieldLabel: 'Consequence {0}',
    ConsequencePostActionFieldNameFieldLabel: 'Field name - Consequence (post action)',
    CreateParentModalSubText:
      'Do you want to turn the site into a parent project? This action is not reversible.',
    CreateParentModalTitle: 'Parent project',
    CreateParentProjectLabel: 'Convert to parent project',
    CreateParentSetupProgressText: 'Setting up parent project',
    RunProjectSetupLabel: 'Run setup wizard',
    RunProjectSetupDialogTitle: 'Run setup wizard again',
    RunProjectSetupDialogSubText:
      'This allows you to select a template, extension, or standard content to be added to the project. This can be used for troubleshooting or to add new functionality.',
    RunProjectSetupDialogProgressText: 'Running setup wizard',
    CurrentPhaseViewNameFieldLabel: 'Display name for current phase',
    DataGroupName: 'Data',
    DefaultGroupByLabel: 'Default grouping',
    DefaultTimeframeEndLabel: 'Default end date',
    DefaultTimeframeEndValue: '{0} months into the future',
    DefaultTimeframeStartLabel: 'Default start date',
    DefaultTimeframeStartValue: '{0} months back in the past',
    DeleteItemLabel: 'Delete',
    DeleteReportButtonText: 'Delete',
    EditItemLabel: 'Edit',
    EditProjectInformationText: 'Edit project information',
    EditReportButtonLabel: 'Edit status',
    EditSiteInformationText: 'Edit site information',
    ExternalUsersConfigInfoMessage:
      'Here fields/columns to be displayed to external users/guests are configured. <br /> <br /> Fields/columns to be displayed to users in the organization are configured in the portfolio site by the administrator.',
    ExternalUsersConfigText: 'Configuration',
    GeneralGroupName: 'General',
    GetSnapshotButtonLabel: 'Open as snapshot',
    HideActionsLabel: 'Hide buttons',
    HideAllActionsLabel: 'Hide all buttons',
    HideParentProjectsLabel: 'Hide parent projects',
    HideChildProjectsLabel: 'Hide child projects',
    HideStatusReportLabel: 'Hide latest status report',
    HookAuthFieldDescription: 'Authentication for execution of calls when changing phase',
    HookAuthFieldLabel: 'Hook authentication',
    HookUrlFieldDescription: 'Url to be called when changing a phase',
    HookUrlFieldLabel: 'Hook url',
    ListNameFieldLabel: 'Listname',
    ListSectionDataErrorMessage:
      'An error occurred while retrieving data for the section. Is it configured correctly? ',
    ListSectionElementIdPrefix: 'section',
    LoadingText: 'Loading {0} ...',
    LookAndFeelGroupName: 'Look and feel',
    ManualConfigurationNotFoundOrInvalid:
      'Manual configuration for Risk matrix not found or has invalid format.',
    ManualConfigurationPathLabel: 'Manual configuration',
    MatrixColorScaleConfigLabel: 'Color configuration',
    MatrixFullWidthLabel: 'Use full width',
    MatrixHeader_Critical: 'Critical',
    MatrixHeader_ExtremelyHigh: 'Extremely high',
    MatrixHeader_ExtremelyLarge: 'Extremely large',
    MatrixHeader_ExtremelyLow: 'Extremely Low',
    MatrixHeader_High: 'High',
    MatrixHeader_Insignificant: 'Insignificant',
    MatrixHeader_Large: 'Large',
    MatrixHeader_Low: 'Low',
    MatrixHeader_Medium: 'Medium',
    MatrixHeader_Moderate: 'Moderate',
    MatrixHeader_Serious: 'Serious',
    MatrixHeader_Small: 'Small',
    MatrixHeader_VeryCritical: 'Very Critical',
    MatrixHeader_VeryHigh: 'Very High',
    MatrixHeader_VeryLarge: 'Very Large',
    MatrixHeader_VeryLow: 'Very Low',
    MatrixHeader_VerySmall: 'Very small',
    MatrixSizeLabel: 'Size',
    MilestoneDateLabel: 'Milestone Date',
    MoveOn: 'Go ahead',
    NavigateToSourceUrlText: 'Go back',
    NewItemLabel: 'New item',
    NewStatusReportLabel: 'New Status Report',
    NewStatusReportTitle: 'New Status Report for {0}',
    No: 'No',
    NoLocalPropertiesListWarningMessage:
      'You must synchronize the project properties before you can configure fields / columns to be displayed to external users / guests. <br /> <br /> This is done by clicking the  "Sync project properties \' button.',
    NoPropertiesMessage: 'No project information found.',
    NoStatusReportsMessage:
      'The status of the project is not yet reported. If you are an owner of the site, you can report status by clicking _New Status Report_ in the menu above. ',
    NotPublishedStatusReport: 'Not published',
    OpportunityMatrixGroupName: 'Opportunity matrix',
    OverrideHeadersLabel: 'Override headers for {0}x{0}',
    ArchiveGroupName: 'Archive',
    ParentProjectsGroupName: 'Parent projects',
    ParentProjectsHeaderText: 'Parent projects',
    ParentProjectsHeaderDescription: 'This lists all programs/parent projects this project belongs to.',
    ChildProjectsGroupName: 'Child projects',
    ChildProjectsHeaderText: 'Child projects',
    ChildProjectsHeaderDescription: 'Child projects managed by this project',
    PhaseChecklistLinkText: 'Go to the phase checklist',
    PhaseFieldFieldLabel: 'Phase Field',
    PhaseSitePageFoundMessage:
      'Changes to this phase will also result in changes to the front page of the project.<br /><br />Page _{0}_ for phase found. This page will be used as the front page for the project. Press **Yes** to continue.',
    PhaseSitePageNotFoundMessage:
      'Dynamic front page is enabled.<br /><br />Page _{0}_ for phase not found, please create. Pressing **Yes** will not change the front page.',
    PleaseWaitText: 'Please wait',
    ProbabilityFieldNameFieldLabel: 'Field name - Probability',
    ProbabilityHeaderFieldLabel: 'Probability {0}',
    ProbabilityPostActionFieldNameFieldLabel: 'Field name - Probability (post action)',
    ProjectDeliveriesGroupName: 'Project deliveries (beta)',
    ProjectInformationDataFetchErrorText: 'An error occurred while retrieving project information.',
    ProjectInformationNoHubAccessMessage: 'The project was loaded without access to the hub site.',
    PropertyValueRenderError: 'Could not display value',
    ProjectInformationStatusReportHeaderText: 'Status report published',
    ProjectStatusNoHubAccessErrorText:
      'Project status cannot be loaded without access to the hub site. If you believe you should have this access, ask an administrator to grant you access to the hub site.',
    ProjectPhasesChangePhaseError: 'An error occured while changing the phase. Please try again or contact an administrator',
    ProjectPhasesFetchDataError: 'You do not have access to view this web part.<br/><br/>You must either have access to the portfolio site, or the project must be exempt from the portfolio site.',
    ProjectStatusDataErrorText: 'No access. You are not allowed to view status reports. ',
    ProjectStatusGroupName: 'Project Status',
    ProjectStatusProjectPropertiesGroupName: 'Project properties (Status Sections)',
    ProjectTimelineErrorFetchText:
      'Error when retrieving the projects data.<br><br>Check that the following SiteId is correct with what is in the "Projects" list. <br><br>- SiteId: {0} <br> - Title: {1} <br><br>Error message: {2}',
    ProjectTimelineNoHubAccessErrorText:
      'Project timeline cannot be loaded without access to the hub site. If you believe you should have this access, ask an administrator to grant you access to the hub site.',
    ProjectTimelineErrorTransformItemText:
      'Error when linking elements to timeline.<br><br>This may be due to a problem with an element in the "Projects" list (duplicate, deleted project), or that a timeline element does not have a valid connection. <br><br>- SiteId: {0} <br> - Title: {1} <br><br>Error message: {2}',
    ProjecttimelineGroupName: 'Project timeline',
    ProjectTimelineItemInfo: '{0} - the projects time course',
    ProjectTimelineListInfoText:
      'The list items for the project are listed here. Here you can edit and add new elements. This will sync to the list on the hubsite. To zoom in/out in timeline: ALT+Mouse wheel',
    PublishedStatusReport: 'Published',
    PublishReportButtonLabel: 'Publish',
    PublishingReportLabel: 'Please wait while the status report is being published...',
    RedoText: 'Change',
    RevertDefaultColorConfigurationText: 'Use default',
    RiskMatrixConfigurationFolder: 'Riskmatrix',
    RowLimitLabel: 'Number of elements',
    OpportunityMatrixConfigurationFolder: 'Opportunitymatrix',
    RiskMatrixGroupName: 'Risk Matrix',
    SaveColorConfigurationText: 'Save',
    SettingsGroupName: 'Settings',
    ShowAllProjectInformationText: 'View all project information',
    ShowFieldExternalUsers: 'Visible to external users',
    ShowProjectDeliveriesLabel: 'Show project deliveries',
    ShowSubTextFieldLabel: 'Show sub text',
    ShowTimelineLabel: 'Show timeline',
    ShowTimelineListCommandsLabel: 'Show command line for list',
    ShowTimelineListLabel: 'Show list',
    SiteAssetsConfigurationFolder: 'SiteAssets/Configuration',
    Skip: 'Skip All',
    SkipSyncToHubLabel: "Don't sync to hub",
    StatusClosed: 'Done',
    StatusElementText: 'Status',
    StatusNotRelevant: 'Not applicable',
    StatusOpen: 'Open',
    StatusReportShowOnlyIconsLabel: 'Show icons only',
    StatusReportTruncateCommentsLabel: 'Truncate comments',
    StatusStillOpen: 'Still open',
    SubTextTruncateLengthFieldLabel: 'Truncate length for sub text',
    SyncProjectPropertiesErrorText:
      'An error occurred while syncing project properties to the portfolio site. For example, it could for instance be an invalid field.',
    SyncProjectPropertiesListProgressDescription: 'Updating Project Properties List',
    SyncProjectPropertiesProgressLabel: 'Synchronizes project properties to the portfolio site',
    SyncProjectPropertiesValuesProgressDescription: 'Please wait...',
    SyncProjectPropertiesValuesProgressLabel: 'Syncing properties',
    SyncPropertiesAfterPhaseChangeFieldLabel: 'Sync propertis after phase change',
    TagFieldLabel: 'Label',
    TasksLinkText: 'Go To Tasks',
    ToggleUncertaintyPostActionLabel: 'Show before/after measures',
    ToggleUncertaintyPostActionOffText: 'Before measures',
    ToggleUncertaintyPostActionOnText: 'After measures',
    TypeLabel: 'Type',
    UnknownConfigurationName: 'Configuration without name',
    UnpublishedStatusReportInfo:
      'There are unpublished status reports. It is therefore not possible to create a new status report. If you are an owner of the site, you can publish or delete unpublished reports using the buttons above.',
    ToggleOnText: 'On',
    ToggleOffText: 'Off',
    UseDynamicHomepageCalloutText:
      'Here you can choose whether the phase selector should be dynamic and use separate pages for each phase. It requires pages for each phase to be created in advance. The names of the pages must be identical to the names of the phases.',
    UseDynamicHomepageFieldLabel: 'Use dynamic homepage',
    UsePhaseHooksCalloutText:
      "Here you can choose whether hooks should be run when changing phases. When changing the phase, a call to the url specified below will be executed. If the call requires authentication, this can be filled in under the url field. The call that is executed sends with the following parameters: 'webUrl' and 'apiKey', both of which are of type 'string'.",
    UsePhaseHooksFieldLabel: 'Use phase selector hooks',
    ArchiveViewTitle: 'Archiving documents and lists',
    ArchiveViewDescription: 'Choose which documents and/or lists you want to archive.',
    ArchiveInformationTitle: 'What to archive?',
    ArchiveInformationText: 'Below you can choose which documents and/or lists to archive. Then press \'<b>Continue</b>\' to proceed.<br /><br />This process retrieves the selected documents and/or lists from the project and logs them for archiving.<br /><br />Each archived item gets its own archive log entry, which is picked up by an archive job that transfers the content to the archive core.<br /><br />The log entry contains information about which items were marked for archiving, who initiated the operation, and when.',
    ArchiveDocumentsSection: 'Documents',
    ArchiveListsSection: 'Lists',
    ArchiveLoadingText: 'Loading archive options...',
    ArchiveContinueText: 'Continue',
    ArchiveNotArchivableText: 'Not archivable',
    ArchiveNotArchivableListText: 'Not archivable, no items',
    ArchiveNotArchivableDocumentText: 'Not archivable: the document type “{0}” is not enabled for archiving',
    ArchiveNotArchivableNoDocumentTypeText: 'Not archivable: the document has no document type assigned',
    ArchiveNotArchivableReasonLabel: 'Reason',
    ArchiveList: 'List: {0}',
    ArchiveDocument: 'Document: {0}',
    ArchivePhaseTransition: 'From {0}-phase to {1}-phase',
    ArchiveManualMessage: 'Manually initiated archiving from project information',
    ArchiveDataErrorText: 'Couldn\'t load the archive options.',
    ArchiveRetryText: 'Try again',
    ArchiveProcessErrorText: 'Some items were not sent to archiving.',
    ArchivePartialNote: 'See the counters above for what completed. You can try again.',
    ArchiveSectionTitle: 'Archiving',
    ArchiveSectionDescription: 'Start archiving of documents and lists from this project, and follow the status of earlier archiving operations.',
    ArchiveStartButtonLabel: 'Start archiving',
    ArchiveItemPreviouslyArchived: 'Last archived {0} — {1}',
    ArchiveItemPreviouslyArchivedAs: 'Last archived as «{0}»',
    ArchiveSummarySelectedLabel: 'selected for archiving',
    ArchiveSummaryEmpty: 'No items selected',
    ArchiveSummaryEmptyHelp: 'Select documents or lists above to start the archiving process.',
    ArchiveSummaryHelpText: 'Marked as «To archive» in the Archive log and picked up by the archiving flow. You will be able to follow the status of the archiving operation from the homepage.',
    ArchiveSummaryDocumentCount: '{0} documents',
    ArchiveSummaryListCount: '{0} lists',
    ArchiveInformationIntro: 'Select documents and lists from this project to send to archiving. Each item is logged in the Archive log and picked up by an archive job.',
    ArchiveInformationShowMore: 'Show more',
    ArchiveInformationShowLess: 'Show less',
    ArchiveConfirmQuestion: 'Send these items for archiving?',
    ArchiveBackButton: 'Back',
    ArchiveSummarySelectedItemsHeader: 'Selected items',
    ArchiveProgressTitle: 'Submitting to archive',
    ArchiveProgressSubText: 'Logging selected items to the Archive log. The actual archiving is performed by an archive job in the background.',
    ArchiveCompletedTitle: 'Archiving complete',
    ArchiveCompletedSubText: 'Selected items have been submitted for archiving. Follow progress in Archiving status on the project page.',
    ArchiveCloseConfirmTitle: 'Discard selection?',
    ArchiveCloseConfirmMessage: 'You have {0} item(s) selected that have not been submitted yet. Close without archiving?',
    ArchiveCloseConfirmDiscard: 'Close anyway',
    ArchiveCloseConfirmKeep: 'Back',
    ArchiveItemModifiedAfterArchive: 'Modified after archiving',
    ArchiveStatusAggregateCounts: '{0} archived · {1} pending · {2} errors',
    ArchiveStatusFilterAll: 'All',
    ArchiveStatusColumnTime: 'Time',
    ArchiveStatusColumnOperation: 'Operation',
    ArchiveStatusColumnMessage: 'Message',
    ArchiveStatusColumnStatus: 'Status',
    ArchiveSectionSelectedCountAria: '{0} of {1} selected',
    ArchiveStatusSummaryLabel: 'archive entries logged',
    ArchiveStatusSummaryHelp: 'Total archive log rows for this project. Click a row for details.',
    ArchiveStatusEmpty: 'No archive entries match the filter.',
    ArchiveRefreshLabel: 'Refresh',
    ArchiveRefreshTooltip: 'Reload data',
    ArchiveTableColumnName: 'Name',
    ArchiveTableColumnModified: 'Last modified',
    ArchiveTableColumnItemCount: 'Item count',
    ArchiveTableColumnDocumentType: 'Document type',
    ArchiveSearchDocumentsPlaceholder: 'Search documents...',
    ArchiveFilterText: 'Quick filter',
    ArchiveFilterArchiveStatusText: 'Archiving status',
    ArchiveFilterDocumentTypeTooltip: 'Filter by document type',
    ArchiveFilterClearText: 'Clear filter',
    ArchiveArchivableText: 'Archivable',
    ArchiveMetaPhaseLabel: 'Phase',
    ArchiveMetaCreatedByLabel: 'Created by',
    ArchiveMetaCreatedLabel: 'Created',
    ArchiveMetaModifiedByLabel: 'Modified by',
    ArchiveMetaModifiedLabel: 'Modified',
    ArchiveNoFilterResultsText: 'No documents match the filter.',
    ErrorArchiving: 'Error archiving: {0}',
    UseArchiveFieldLabel: 'Use archive functionality',
    ViewsGroupName: 'Views',
    ViewVersionHistoryText: 'View version history',
    ViewXmlFieldLabel: 'View XML query',
    WebPartNoAccessMessage:
      'You do not have access to view this web part. <br/> <br/> You must either have access to the portfolio site or the project must be free from the portfolio site.',
    WidthFieldLabel: 'Width (pixels)',
    Yes: 'Yes',
    ProjectNewsWebPartTitle: 'Project News',
    DialogTitle: 'Create a news article',
    DialogDescription: 'Here you can create a news article for the project. The news article will be linked to the project. In the "Template" dropdown menu, you can select a template that will pre-fill the news article. Here, it is possible to get a preview of the content by clicking the "Preview selected template" button.',
    NewsHubLevelWarningTitle: 'Warning',
    NewsHubLevelWarningText: 'You are about to create a news article at the portfolio level of Prosjektportalen. The content will be visible to everyone with access to Prosjektportalen in your organization, and SharePoint may send email notifications about the news to colleagues who follow the portfolio site.',
    CreatingNewArticleStatus: 'Creating news article...',
    NewsCreatedSuccessfully: 'News article created successfully.',
    NewsTitleLabel: 'Title',
    NewsTitlePlaceholder: 'Enter value here',
    CreateButtonLabel: 'Create',
    CreateNewsLinkLabel: 'Create an article',
    NewsCreateError: 'Something went wrong: ',
    NewsTitleRequired: 'Title is required to create a news article.',
    TemplateRequired: 'Please select a template to create the news article.',
    TemplatePlaceholder: 'Select an option',
    TemplateLabel: 'Template',
    NoRecentNews: 'Here you can create news articles for the project. There are currently no news articles.',
    ShowMoreNews: 'Show more',
    ShowLessNews: 'Show less',
    ModifiedTooltipText: 'Modified {0}',
    MaxVisibleNewsLabel: "Number of visible news articles (before 'Show more')",
    MaxVisibleNewsDescription:
      'Set the number of news articles to display in the web part.',
    NewsFolderNameDescription: 'The name of the folder where news articles should be stored.',
    NewsFolderNameDefault: 'Project News',
    NewsFolderNameLabel: 'News Folder Name',
    FileNameRequired: 'File name is required.',
    FileNameTooLong: 'File name is too long.',
    FileNameInvalid: 'File name contains invalid characters.',
    FileNameNoLeadingTrailingSpaces: 'File name cannot have leading or trailing spaces.',
    FileNameReserved: 'File name is reserved.',
    NewsFolderError: "Failed to check or create news folder: ",
    LoadingLabel: 'Loading news articles...',
    GenericErrorMessage: 'Something went wrong. Please try again later or contact support.',
    NewsCreateDuplicateFileError: 'A news article with the same title already exists. Please choose a different title.',
    PreviewLabel: 'Preview selected template',
    NoPreviewAvailable: 'Preview unavailable',
    SynchronizingProjectPropertiesToPortfolioSiteErrorText: 'An error occurred while syncing project properties to the portfolio site. Please try again later.',
    NewsCreateSuccess: 'News article created successfully',
    SiteUrlLabel: 'Site URL',
    SiteUrlDescription: 'Enter the URL of the site where the news should be published.',
    ShowCommentText: 'Show comment',
    HideCommentText: 'Hide comment',
    ArchiveStatusHeaderText: 'Archive Status',
    ArchiveStatusDetailedMessage: 'Last archive activity: {0}',
    ArchiveStatusDocumentCount: '{0} document(s)',
    ArchiveStatusListCount: '{0} list(s)',
    ArchiveStatusTotalCount: 'Sent to archive: ',
    DynamicList: {
      DefaultTitle: 'Dynamic List',
      SelectAll: 'Select all',
      SelectRow: 'Select row',
      NoListSelected: 'No list selected',
      NoListSelectedMessage: 'Please select a list in the web part properties to display content.',
      RefetchingData: 'Refreshing and fetching data...',
      SiteIdFieldsMissing: 'Site ID related fields not found',
      SiteIdFieldsMissingMessage: 'The Site ID (GtSiteId) and Site Title (GtSiteTitle) fields must exist on the list/document library to use this functionality. Use the button in the web part properties to add the fields, or create them manually on the list/document library.',
      NoItemsToShow: 'No items to show',
      NoColumnsToShow: 'No columns to show',
      ConfirmDelete: 'Are you sure you want to delete {0} item?',
      ConfirmDeleteMultiple: 'Are you sure you want to delete {0} items?',
      ConfigurationError: 'Configuration error',
      HookUrlNotConfigured: 'Hook URL is not configured for this action.',
      NoItemsSelected: 'No items selected',
      NoItemsSelectedMessage: 'Please select at least one item to perform this action.',
      ActionCompleted: 'Action completed. {0}',
      ActionFailed: 'Action failed',
      CorsError: 'CORS error: The server does not allow requests from this domain. Contact your administrator to configure CORS headers.',
      CouldNotExecuteAction: 'Could not execute action: {0}',
      Back: 'Back',
      BackToListView: 'Back to list view',
      New: 'New',
      NewDocumentOrUpload: 'Create new document or upload file',
      WordDocument: 'Word document',
      ExcelWorkbook: 'Excel workbook',
      PowerPointPresentation: 'PowerPoint presentation',
      UploadFile: 'Upload file',
      NewItem: 'New item',
      CreateNewItem: 'Create a new item',
      EditItem: 'Edit item',
      EditSelectedItem: 'Edit selected item',
      SearchIn: 'Search in {0}...',
      Search: 'Search',
      ExportSelected: 'Export {0} selected item{1} to Excel',
      ExportToExcel: 'Export to Excel',
      FolderView: 'Folder view',
      FlatView: 'Flat view',
      Default: ' (Default)',
      SelectView: 'Select view',
      SelectViewTooltip: 'Select a view to display',
      Refresh: 'Refresh',
      Delete: 'Delete',
      DeleteSelectedItems: 'Delete selected items',
      ShowHideFilters: 'Show/hide filters',
      NewWordDocument: 'New Word document {0}.docx',
      NewExcelWorkbook: 'New Excel workbook {0}.xlsx',
      NewPowerPointPresentation: 'New PowerPoint presentation {0}.pptx',
      NoItemFound: 'No item found',
      NoItemsMessage: 'There are no items to display',
      Untitled: 'Untitled',
      Documents: 'Documents',
      NoDocumentsInFolder: 'No documents to display in selected folder, go back by navigating in the menu above',
      NoDocuments: 'No documents to display',
      DragFilesOrClick: 'Drag files here or click to upload',
      UploadingFiles: 'Uploading files...',
      DropFilesToUpload: 'Drop files here to upload',
      PleaseWait: 'Please wait...',
      FilesWillBeUploaded: 'Files will be uploaded to the current folder',
      Uploading: 'Uploading...',
      SelectFiles: 'Select files',
      ContentLoadedSuccessfully: 'Content loaded successfully',
      ProcessingFailed: 'Processing failed',
      TimeoutError: 'Timeout - processing took too long',
      CouldNotFetchContent: 'Could not fetch content: {0}',
      PollingFailed: 'Polling failed: {0}',
      FetchError: 'Fetch error',
      NoItemsSelectedError: 'No items selected. Please select at least one item.',
      NoIframeContent: 'No iframe content or poll URL received',
      ExecutionError: 'Execution error: {0}',
      Error: 'Error',
      SendingData: 'Sending data for selected item...',
      FetchingContent: 'Fetching content from server...',
      Cancel: 'Cancel',
      Close: 'Close',
      NoItemsFound: 'No items found',
      ListTable: 'List table',
      GroupByColumn: 'Group by {0}',
      FolderNavigation: 'Folder navigation',
      HiddenFromView: 'Hidden from view'
    }
  }
})
