...
import { getFluentIcon, FluentIconName } from 'pp365-shared-library/lib/icons'
 ...
 * @param onOpenChange Callback for when the open state of the menu changes
 *
      ...
      if (item.subMenuProps?.items) {
        return (
          <Menu key={item.key}>
            ...
            <MenuPopover>
              {item.subMenuProps?.items.map((subItem) => renderMenuItem(subItem, onOpenChange))}
            </MenuPopover>
          </Menu>
        )
      }
