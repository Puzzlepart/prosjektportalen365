...
export default new class SPDataAdapter extends SPDataAdapterBase<ISPDataAdapterConfiguration> {
            ...
            for (let i = 0; i < fieldToSync.length; i++) {
                ...
                switch (fld.TypeAsString) {
                    case 'TaxonomyFieldType': case 'TaxonomyFieldTypeMulti': {
                        ...
                        break
                    case 'User': {
                        const [siteUser] = siteUsers.filter(u => u.Id === fieldValues[`${fld.InternalName}Id`])
                        const user = siteUser ? await this.entityService.web.ensureUser(siteUser.LoginName) : null
    ...
    public async getTermFieldContext(fieldName: string) {
        const phaseField = await this.sp.web.fields.getByInternalNameOrTitle(fieldName)
            .select('InternalName', 'TermSetId', 'TextField')
            .usingCaching()
            .get<{ InternalName: string; TermSetId: string; TextField: string }>()
        const phaseTextField = await this.sp.web.fields.getById(phaseField.TextField)
            .select('InternalName')
            .usingCaching()
