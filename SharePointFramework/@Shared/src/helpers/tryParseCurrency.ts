 ...
 */
export function tryParseCurrency(str: string, fallback: string = '', currencyPrefix = 'kr', minimumFractionDigits = 0, maximumFractionDigits = 0): string {
  const parsed = tryParseFloat(str, fallback)
  if (parsed === fallback) return fallback
  return `${currencyPrefix} ` + (parsed as number).toLocaleString(undefined, {
