...
var templatesReplaceMap = getFileContent(`${templatesPath}/.replace-map.json`)
var templateReplaceValues = Object.keys(currentChannelConfig.spfx.solutions).reduce((acc, key) => {
...
/**
 * Replace tokens in the given template path. The tokens are defined in the .replace-map.json file
...
function replaceTokensInTemplate(templatePath) {
    for (var key in templatesReplaceMap) {
        var token = templatesReplaceMap[key]
        var replacement = templateReplaceValues[token]
        replace({
            regex: key,
            replacement,
            paths: [templatePath],
