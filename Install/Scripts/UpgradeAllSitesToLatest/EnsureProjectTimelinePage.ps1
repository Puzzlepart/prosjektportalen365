...
if ($global:__PreviousVersion -lt $TargetVersion) {
    ...
    if ($null -eq $ExistingNodes) {
        ...
        if ($null -eq $ExistingNode) {
            ...
            $ExistingPage = Get-PnPClientSidePage -Identity "Prosjekttidslinje" -ErrorAction SilentlyContinue
            if ($null -eq $ExistingPage) {
                ...
                Write-Host "`t`t`tThe project timeline page already exists" -ForegroundColor Green
            }
        }
        else {
            Write-Host "`t`tThe site already has the project timeline page in navigation" -ForegroundColor Green
        }
    }   
}