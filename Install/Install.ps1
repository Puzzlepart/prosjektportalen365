...
if (-not $SkipSiteDesign.IsPresent) {
    ...
    Try {
        ...
        if ($null -ne $SiteDesign) {
            $SiteDesign = Set-PnPSiteDesign -Identity $SiteDesign -SiteScriptIds $SiteScriptIds -Description "Samarbeid i et prosjektomr%C3%A5de fra Prosjektportalen" -Version "1"
        }
        else {
            $SiteDesign = Add-PnPSiteDesign -Title $SiteDesignName -SiteScriptIds $SiteScriptIds -Description "Samarbeid i et prosjektomr%C3%A5de fra Prosjektportalen" -WebTemplate TeamSite
        }
